...
| 20 | Sửa đơn tạo payload cũ, sync sai | 🔴 High | Sync | `SyncQueueManager.enqueue()` thấy trùng `clientRequestId` → trả về id cũ, KHÔNG cập nhật `payloadJson`; + 2 root cause bổ sung phát hiện qua retest (xem bảng dưới) | 🛠️ **FIXED** `7e892c36`, `ecb10121`, `90be1508`, `e4b6cb5f` — chưa test on-device |
...
| 20 | Sửa đơn tạo payload cũ, sync sai | `7e892c36`, `ecb10121`, `90be1508`, `e4b6cb5f` | `SyncQueueManager.enqueue()` cập nhật `payloadJson` khi row non-terminal. **Fix bổ sung 1** (`ecb10121`): tester phát hiện server vẫn nhận bản cũ sau sync — root cause thật là `SyncQueueDrainer.drain()` snapshot cả batch 1 lần qua `pickPending()`, dispatch dùng snapshot cũ nếu edit/issue re-enqueue kịp ghi đè DB sau snapshot nhưng trước lượt dispatch của row đó. Đã thêm `SyncQueueManager.findById()` + đọc lại ngay trước khi dispatch. **Fix bổ sung 2** (`90be1508`): "offline sửa đơn → xuất đơn → khi online lại đơn vẫn ở ĐH chưa xuất" — đơn đã issue (docStatus=2) local bị dedupe filter bỏ sót vì dedupe-key set chỉ build từ subset đơn `docStatus<=1`, để lọt 1 row `cachedUndeliveredTransactions` cũ (snapshot server trước khi issue) tái xuất hiện như ghost card ở tab "chưa xuất". Đã sửa dedupe-key set bao phủ TOÀN BỘ đơn offline. **Fix bổ sung 3** (`e4b6cb5f`, root cause khác hẳn, xác nhận bằng 4 ảnh chụp màn hình của tester): đơn issue offline SAU KHI CREATE payload đã sync SUCCESS lên server — `SyncQueueManager.patchPayload()` chỉ ghi khi row còn non-terminal nên docStatus=2 bị rớt âm thầm (comment cũ tự thừa nhận "a Phase 2 path will handle re-posting via API" nhưng chưa từng viết). Server có mã đơn thật ("SO...") nhưng docStatus vẫn = 0 vì chưa bao giờ nhận được. Đã thêm cờ `pendingIssueConfirmation` (schema v41) + `SalesOrdersOfflineRepository.reconcileUnconfirmedIssues()` gọi lại `updateProcessOrd` (đúng API luồng Online "Xuất đơn" đang dùng) khi online, wire vào `SyncQueueDrainer.drain()`. Hợp lệ vì `SalesOrders.id` CHÍNH LÀ `SalesOrder.Id` phía server (`ProcessSalesOrderOfflineJob` persist `id: payload.OrderId`) — không cần chờ có server-assigned id riêng |
| 16 | Tồn tặng đủ — hàng tặng không ràng buộc tồn kho | `fba61b7c` | Guard tồn kho trong `confirm_order_bloc.dart` giờ duyệt cả `freePromotions`, không chỉ `products`. **Tester retest với step khác** ("đơn hàng tặng/sample" qua `SampleListScreen`, không phải "hàng tặng trong đơn thường") — guard có chạy tới nhưng `stock_snapshots` local chỉ sync SP saleable (Attribute03=Z002/Z007/Z013), SP tặng thường không có trong đó nên check luôn no-op. **Đang chờ xác nhận nguồn tồn kho SP tặng trước khi code tiếp** |
| 11 | Bộ lọc lỗi khi offline | `db998717`, `9d20fdb1`, BE `a136915a2` + FE `32bd3db1` | `fetchRoutePlanMaster` fallback ban đầu dùng `SalesRoutesDao` (SAI). **Fix bổ sung 1** (`9d20fdb1`): "Tuyến bán hàng" Online thực ra là list Thứ cố định (Mon-Sun) — đã sửa đúng nguồn; `fetchRouteStatusMaster` hardcode "Có/Không đơn hàng"; wire filter `ordStatus`. **Fix bổ sung 2 — Chợ + Thuộc tính KH (đã xong, 5/5)**: thêm 2 pull module mới cả BE (`markets`, `customerAttributeBySalesChannels`) lẫn FE (bảng + DAO + pull handler), thêm `marketId` vào `salesRouteDetails` payload (BE) và cột `marketId` vào `sales_route_visits` (FE, schema v40), wire filter `market`/`attribute` vào `_fetchSalesRouteFromDrift`. **Chưa build/test on-device** |
| 22 | KPI cache lẫn nhiều tài khoản | `dd01d164`, `7ad45388`, BE `69caaf37e` + FE `53d15293` | `GeneralApp.clearDataUserLogin()` nhận `kpiSnapshotsDao`, xóa cache khi "đổi tài khoản". **Fix bổ sung 1** (`7ad45388`): tester xác nhận Báo cáo chỉ tiêu đúng nhưng Dashboard vẫn sai — Dashboard đọc thêm 3 bảng (`visit_statuses`, `visit_captures`, `sku_per_order_snapshots`) chưa được clear (2 bảng đã có hàm `clear()` sẵn nhưng chưa từng gọi, 1 bảng phải thêm mới) — đã wire cả 3 vào. **Fix bổ sung 2** (root cause khác hẳn, phát hiện qua 4 ảnh chụp Dashboard/Báo cáo Online vs Offline trên cùng tài khoản): KPI lệch nhau giữa Online/Offline VÀ giữa Dashboard/Báo cáo offline với nhau — Check-in 4/17 (Online) vs 4/73 (Offline, do `VisitStatusDao.getAll()` không lọc theo ngày, cộng dồn mọi KH từng sync); SKU/Đơn hàng 1,2 (Online) vs 0 (Offline, do `sku_per_order_snapshots` combine local chưa sync hôm đó); "Còn lại" ITEM_INVOICE 58,43 (Online, đọc `fs_rp_dailykpi.psql` — công thức cross-KPI với NO_INVOICE, KHÔNG phải trừ tuyến tính) vs 53 (Dashboard offline) vs 53,67 (Report offline, = 55-1,33 đúng công thức SAI). Đã đọc `.psql` xác nhận công thức thật, rồi **thay vì replicate công thức phức tạp trên client, đẩy thẳng giá trị server đã tính sẵn qua sync**: BE (`69caaf37e`) thêm `KpiSnapshotEntryDto.Remaining` + `KpiSnapshotDto.AverageSkuPerOrder` vào payload `kpiSnapshot` (trước đây `Remaining` bị chôn trong `AttributesJson`, không có field riêng); FE (`53d15293`, schema v41→v42) thêm cột `remaining`/`averageSkuPerOrder` vào Drift, parse ở pull handler, và đổi cả `DashboardBloc._loadDashboardFromDrift` lẫn `ReportKpiBloc._toKpiModel` dùng giá trị synced thay vì tự tính lại; đồng thời đổi thứ tự ưu tiên Check-in sang `kpi_snapshots.visited/total` (đúng, cùng nguồn Online) thay vì `VisitStatusDao` (sai). **Chưa build/test on-device** |
...
| 13 | Phiếu bán hàng thiếu field Online/Offline | `08aa64f9`, `70c3c25e`, `75319303`, `9fa208d5` | Thêm `SalesOrderInfo.originMode` (local-only, không qua JSON), render dòng "Nguồn đơn". **3 fix bổ sung sau khi tester phát hiện dữ liệu sai/thiếu**: (1) `70c3c25e` — `SalesOrdersPullHandler` ghi đè `originMode` về `'ONLINE'` mỗi lần pull reconciliation; (2) `75319303` — xem đơn lúc thiết bị Online lại (đơn đã sync) đi qua nhánh server-fetch thuần, chưa backfill từ Drift; (3) `9fa208d5` — **root cause thật của "field biến mất ngay lúc vừa tạo đơn"**: `SalesInvoiceForm.initState` bỏ qua toàn bộ enrichment khi có `salesOrderResponse` truyền thẳng vào (luồng ConfirmOrder→ReviewOrder→SalesInvoiceScreen ngay sau khi lưu đơn), và `SalesOrdersOfflineRepository._localSuccess` chưa từng stamp `originMode` lên response trả về dù Drift row vừa ghi đã có `originMode='OFFLINE'`. Đã fix stamp `originMode` ngay tại `_localSuccess`. Ghi chú thêm: máy ảo test có network thật (không airplane mode) nên nhiều lần "test offline" thực ra đi qua nhánh Online — không ảnh hưởng fix nhưng dễ gây nhầm lẫn khi tái hiện |
| 18 | Thiếu badge Online/Offline | `bcec8bbd`, `70c3c25e`, `acd8f958`, `087f4145`, `5c805edf` | Tab "ĐH chưa xuất": ban đầu hardcode "Online" cho card server-fetched — đã sửa thành tra Drift thật + derive badge đúng (`acd8f958`), và theo yêu cầu tester đổi nhãn "Đã đồng bộ"→"Offline" nền xanh lá riêng cho card này (`087f4145`). **Fix bổ sung** (`5c805edf`): tester chụp ảnh xác nhận tab "ĐH đã xuất" vẫn thiếu badge hoàn toàn — đã thêm badge tương tự (tra `salesOrdersDao` theo id/clientRequestId, derive qua `TransactionStatusBadge.derive`) vào `delivered_orders_form.dart`. **Không có test mới cho phần UI** (widget chưa có test harness sẵn) — logic derive dùng lại `TransactionStatusBadge.derive` đã có test riêng. Việc MERGE 2 list online/offline thành 1 danh sách duy nhất (thay vì 2 khối riêng) vẫn ở hàng đợi P2, ngoài phạm vi fix badge này |
...
P1 — High, cần fix sớm (data integrity / block core flow):
  Bug 20 — ✅ FIXED (7e892c36, ecb10121, 90be1508, e4b6cb5f, chưa test on-device) — Sửa đơn: payload cũ không cập nhật
  ...
  Bug 22 — ✅ FIXED (dd01d164, 7ad45388, BE 69caaf37e, FE 53d15293, chưa test on-device) — KPI cache lẫn tài khoản + lệch số Online/Offline
