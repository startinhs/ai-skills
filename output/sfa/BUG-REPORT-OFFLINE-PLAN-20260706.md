...
| 22 | KPI cache lẫn nhiều tài khoản | `dd01d164`, `7ad45388` | `GeneralApp.clearDataUserLogin()` nhận `kpiSnapshotsDao`, xóa cache khi "đổi tài khoản". **Fix bổ sung**: tester xác nhận Báo cáo chỉ tiêu đúng nhưng Dashboard vẫn sai — Dashboard đọc thêm 3 bảng (`visit_statuses`, `visit_captures`, `sku_per_order_snapshots`) chưa được clear (2 bảng đã có hàm `clear()` sẵn nhưng chưa từng gọi, 1 bảng phải thêm mới) — đã wire cả 3 vào |
