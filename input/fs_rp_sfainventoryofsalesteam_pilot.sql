...
BEGIN
    ...
    product_info AS (
        SELECT
            ...
            CASE
                WHEN p."HierarchyL02Code" = '0101' THEN COALESCE(ph_type_l03."Description", ' ')
                ELSE COALESCE(ph_type_l04."Description", ' ')
            END AS type_description,
            COALESCE(p."Attribute03", '') AS "Attribute03"
        FROM "Products" p
        LEFT JOIN "ProductHierarchies" ph_brand
            ON ph_brand."Id" = p."HierachyL02Id" AND ph_brand."NodeLevel" = '1'
        LEFT JOIN "ProductHierarchies" ph_type_l03
            ON ph_type_l03."Id" = p."HierachyL03Id" AND ph_type_l03."NodeLevel" = '2'
        LEFT JOIN "ProductHierarchies" ph_type_l04
            ON ph_type_l04."Id" = p."HierachyL04Id" AND ph_type_l04."NodeLevel" = '3'
        LEFT JOIN "SKUTypes" ph_type_l05
            ON ph_type_l05."Id" = P."HierachyL05Id"
        WHERE --p."Code" NOT LIKE '149%' AND 
        p."IsDeleted" = FALSE
    ),
    ...
    beginning_qty AS (
        ...
        SELECT 
            ...
            it."UOMCode",
            SUM(it."BaseQuantity"::numeric)::numeric AS base_qty,
    ...
    issued_qty AS (
        ...
        SELECT 
            ...
            sop."UOMCode",
            SUM(CASE WHEN so."TypeOfScreen" = 'SRO' THEN -1 ELSE 1 END * sop."Quantity"::numeric)::numeric AS base_qty,
            v_date AS "TransactionDate"
        FROM "SalesOrders" so
    ...
    not_issued_qty AS (
        ...
        SELECT 
            ...
            sop."BinCode" AS "WarehouseCode", 
            sop."UOMCode",
            SUM(sop."Quantity"::numeric)::numeric AS base_qty,
            v_date AS "TransactionDate"
        FROM "SalesOrders" so
        INNER JOIN "SalesOrderProducts" sop ON so."Id" = sop."SalesOrderId"
    ...
    SELECT
        ...
        ROUND(
            CASE v_tab_type
                WHEN 1 THEN COALESCE(bq.base_qty, 0)::numeric
                WHEN 2 THEN COALESCE(iq.base_qty, 0)::numeric
                WHEN 3 THEN COALESCE(niq.base_qty, 0)::numeric
                WHEN 4 THEN COALESCE(bq.base_qty, 0)::numeric - COALESCE(iq.base_qty, 0)::numeric - COALESCE(niq.base_qty, 0)::numeric
                WHEN 5 THEN COALESCE(bq.base_qty, 0)::numeric - COALESCE(iq.base_qty, 0)::numeric
                ELSE 0
            END::NUMERIC, 4
        ) AS "Quantity",
        v_date AS "TransactionDate"
