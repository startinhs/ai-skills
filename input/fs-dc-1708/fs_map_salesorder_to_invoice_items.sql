...
BEGIN
    ...
    FROM (
        ...
        SELECT
            ...
            -- C#: BuildPAExpandedItemsAsync → ĐVT lấy theo ProductComponents.UOM (UnitOfMeasures.Description),
            -- fallback UOMName → UOM; dòng KM không rã component thì giữ ĐVT của dòng cha
            CASE WHEN e."ComponentProductId" IS NOT NULL
                 THEN COALESCE(uomc."Description", NULLIF(e."UOMName", ''), NULLIF(e."UOM", ''), '')
                 ELSE op."Unit"
            END AS "Unit",
            ...
            0::INTEGER AS "IsSum"  -- C#: IsFreeItem=true → IsSum=1
        FROM ordered_products op
        ...
        LEFT JOIN "UnitOfMeasures" uomc ON uomc."Code" = e."UOM"
        ...
        SELECT
            ...
            (-2)::numeric AS "VatRate",
            NULL::numeric AS "VatAmount",  -- C#: VatRate=-2 is basic rate → vatAmount=null
            0::INTEGER AS "IsSum"  -- C#: IsFreeItem (PA expanded) → IsSum=1
        FROM ordered_products op
    ...
    -- HÀNG KM ED (Promo ED - IsKMDiscountNotReducePrice): IsSum=2, giống C# promoEdDiscountItems
    -- Tên: "Chiết khấu thương mại <PromotionCode>"
    -- Thành tiền: SUM(DiscountAmount) group theo PromotionCode
    -- Nhiều DepartmentOfIndustryandTradeCode cùng PromotionCode → gộp lại thành 1 dòng
    promo_ed_grouped AS (
        SELECT
            TRIM(COALESCE(sod."PromotionCode", '')) AS promo_code,
            MIN(COALESCE(sod."Idx", 2147483647)) AS min_idx,
            SUM(COALESCE(sod."DiscountAmount", 0)) AS line_amount,
            -- lấy SalesProductCode đầu tiên trong group để resolve VatRate
            (ARRAY_AGG(sod."SalesProductCode" ORDER BY COALESCE(sod."Idx", 2147483647), sod."CreationTime"))[1] AS first_sales_product_code
        ...
        GROUP BY TRIM(COALESCE(sod."PromotionCode", ''))
    ),
    product_items_promo_ed AS (
        ...
        -- C#: ResolvePromoEdDiscountLineVatRate: SalesProductCode (comma-sep) → match SalesOrderProducts.ProductCode → prefer TaxPercent>0
        vr.tax_rate::NUMERIC AS "VatRate",
        CASE
            WHEN vr.tax_rate IS NULL THEN NULL
            WHEN vr.tax_rate IN (-1, -2, -4, 0, 5, 8, 10) THEN NULL
            ELSE ROUND(g.line_amount * vr.tax_rate / 100, 0)
        END::NUMERIC AS "VatAmount",
            2::INTEGER AS "IsSum",  -- IsSum=2 giống C# mapper (KM ED)
            FALSE AS "IsPA"
        FROM promo_ed_grouped g
        LEFT JOIN LATERAL (
            SELECT COALESCE(
                -- ưu tiên TaxPercent > 0
                (SELECT sop2."TaxPercent"
                 FROM UNNEST(STRING_TO_ARRAY(TRIM(g.first_sales_product_code), ',')) AS codes(code)
                 JOIN "SalesOrderProducts" sop2
                     ON TRIM(sop2."ProductCode") = TRIM(codes.code)
                     AND sop2."SalesOrderId" = v_sales_order_id
                 WHERE sop2."TaxPercent" > 0
                 ORDER BY sop2."Idx", sop2."Id"
                 LIMIT 1),
                -- fallback: first candidate bất kỳ
                (SELECT sop2."TaxPercent"
                 FROM UNNEST(STRING_TO_ARRAY(TRIM(g.first_sales_product_code), ',')) AS codes(code)
                 JOIN "SalesOrderProducts" sop2
                     ON TRIM(sop2."ProductCode") = TRIM(codes.code)
        ...
        ) vr ON (g.first_sales_product_code IS NOT NULL AND TRIM(COALESCE(g.first_sales_product_code, '')) <> '')
    ),
    ...
    SELECT
        CASE
    WHEN ai."SortGroup" IN (2, 5, 6, 8, 9) THEN 0 -- dòng header/ghi chú/KM ED (không đánh số)
